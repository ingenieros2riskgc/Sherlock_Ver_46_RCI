...
{
    public partial class ParRieTratamiento : System.Web.UI.UserControl
    {
        ...
        private void loadGrid()
        {
            ...
            grid.Columns.Add("IdTratamiento", typeof(string));
            grid.Columns.Add("NombreTratamiento", typeof(string));
            InfoGrid = grid;
        ...
        private void loadInfo()
        {
            ...
            dtInfo = cParametrizacionRiesgos.loadInfoTratamiento();
            if (dtInfo.Rows.Count > 0)
            {
                for (int rows = 0; rows < dtInfo.Rows.Count; rows++)
                {
                    InfoGrid.Rows.Add(new Object[] {
                        dtInfo.Rows[rows]["IdTratamiento"].ToString().Trim(),
                        dtInfo.Rows[rows]["NombreTratamiento"].ToString().Trim()
                        });
                }
            ...
            switch (lbldummyOkNo.Text.Trim())
            {
                case "Tratamiento":
                    try
                    {
                        ...
                        loadGrid();
                        loadInfo();
                        Mensaje("Tratamiento eliminado con éxito.");
                    }
                    catch (Exception ex)
                    {
        ...
        protected void ImageButton2_Click(object sender, ImageClickEventArgs e)
        {
            try
            {
                ...
                {
                    cParametrizacionRiesgos.agregarRegistroTratamiento(Sanitizer.GetSafeHtmlFragment(TextBox1.Text.Trim()));
                    resetValuesCampos();
                    loadGrid();
                    loadInfo();
        ...
        protected void ImageButton3_Click(object sender, ImageClickEventArgs e)
        {
            try
            {
                ...
                {
                    cParametrizacionRiesgos.modificarRegistroTratamiento(Sanitizer.GetSafeHtmlFragment(TextBox1.Text.Trim()), InfoGrid.Rows[RowGrid]["IdTratamiento"].ToString().Trim());
                    resetValuesCampos();
                    loadGrid();
                    loadInfo();
        ...
        {
            cParametrizacionRiesgos.borrarRegistroTratamiento(InfoGrid.Rows[RowGrid]["IdTratamiento"].ToString().Trim());
        }

        private void detalleRegistro()
        {
            TextBox1.Text = InfoGrid.Rows[RowGrid]["NombreTratamiento"].ToString().Trim();
