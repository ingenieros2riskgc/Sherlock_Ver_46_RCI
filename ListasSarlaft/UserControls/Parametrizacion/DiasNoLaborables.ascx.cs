...
{
    public partial class DiasNoLaborables : System.Web.UI.UserControl
    {
        ...
        protected void GridView1_SelectedIndexChanged(object sender, EventArgs e)
        {
            ...
            if (GridView1.SelectedRow.RowType == DataControlRowType.DataRow)
            {
                ...
                txtFNL.Text = GridView1.SelectedRow.Cells[0].Text.Trim();
                if (GridView1.SelectedRow.Cells[1].Text.Trim() == "&nbsp;")
                    txtDesc.Text = "";
                else
                    txtDesc.Text = GridView1.SelectedRow.Cells[1].Text.Trim();
        ...
        protected void GridView1_RowCommand(object sender, GridViewCommandEventArgs e)
        {
            ...
            try
            {
                if (e.CommandName == "Eliminar")
                {
                    ...
                    txtFNL.Text = row.Cells[0].Text.Trim();
                }
            }
        ...
        protected void btnImgokEliminar_Click(object sender, EventArgs e)
        {
            ...
            try
            {
                SqlDataSource1.DeleteParameters["FechaNoLaborable"].DefaultValue = Sanitizer.GetSafeHtmlFragment(txtFNL.Text);
        ...
        protected void btnImgActualizar_Click(object sender, ImageClickEventArgs e)
        {
            ...
            else
            {
                if (VerificarCampos())
                {
                    try
                    {
                        SqlDataSource1.UpdateParameters["FechaNoLaborable"].DefaultValue = Sanitizer.GetSafeHtmlFragment(txtFNL.Text);
                        SqlDataSource1.UpdateParameters["Descripcion"].DefaultValue = Sanitizer.GetSafeHtmlFragment(txtDesc.Text);
        ...
        protected void btnImgInsertar_Click(object sender, ImageClickEventArgs e)
        {
            if (VerificarCampos())
            {
                ...
                try
                {
                    SqlDataSource1.InsertParameters["FechaNoLaborable"].DefaultValue = Sanitizer.GetSafeHtmlFragment(txtFNL.Text);
                    SqlDataSource1.InsertParameters["Descripcion"].DefaultValue = Sanitizer.GetSafeHtmlFragment(txtDesc.Text);
        ...
        protected Boolean VerificarCampos()
        {
            ...
            if (ValidarCadenaVacia(Sanitizer.GetSafeHtmlFragment(txtFNL.Text)))
            {
                err = false;
                omb.ShowMessage("Debe ingresar la Fecha No Laborable.", 2, "Atención");
                txtFNL.Focus();
            }
            else if (ValidarCadenaVacia(Sanitizer.GetSafeHtmlFragment(txtDesc.Text)))
            {
                err = false;
